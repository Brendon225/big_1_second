...
def run_training(config_path: str) -> Dict[str, Any]:
    ...
    model = build_stage1_model(
        ...
        instance_pooling=config.get("instance_pooling", "mean"),
        use_entity_markers=config.get("use_entity_markers", False),
        use_prototype_fusion=config.get("use_prototype_fusion", False),
        prototype_fusion_alpha=config.get("prototype_fusion_alpha", 0.0),
